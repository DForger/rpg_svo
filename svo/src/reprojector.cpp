...
{
  // reproject map means reproject map point
  resetGrid();
       ...
       ++it_frame, ++n)
  {
    ...
    // iterate all features in reference frame
    for (auto it_ftr = ref_frame->fts_.begin(), ite_ftr = ref_frame->fts_.end(); it_ftr != ite_ftr; ++it_ftr)
    {
      ...
      // project to frame, and add to gridcells
  ...
  // Now project all point candidates into cells
...
{
  // XXX: point quality is determined by point type
  // point types {
  //  TYPE_DELETED,
  //  TYPE_CANDIDATE,
  //   TYPE_UNKNOWN,
...
{
  // for all features in the cell, sorting feature quality for high to low
  // test from high quality feature to low quality feature
  // if a high quality feature is successfully projected into the cell
  // skip the rest feature and return
  ...
  while (it != cell.end())
  {
    ...
    // for the point successfully reprojected multiple times, promote to good
    it->pt->n_succeeded_reproj_++;
    if (it->pt->type_ == Point::TYPE_UNKNOWN && it->pt->n_succeeded_reproj_ > 10)
      it->pt->type_ = Point::TYPE_GOOD;

    // a feature is a 2d projection of map point, it might in different level
    // the default feature type is corner
    ...
    // XXX: what is a refer feature?
    if (matcher_.ref_ftr_->type == Feature::EDGELET)
    {
      // apply affine warp to the gradient direction
...
bool Reprojector::reprojectPoint(FramePtr frame, Point* point)
{
  ...
  if (frame->cam_->isInFrame(px.cast<int>(), 8))  // 8px is the patch size in the matcher
  {
    // cell idx
    const int k =
        static_cast<int>(px[1] / grid_.cell_size) * grid_.grid_n_cols + static_cast<int>(px[0] / grid_.cell_size);
